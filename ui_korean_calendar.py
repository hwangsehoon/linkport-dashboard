  ...
  function translate() {
    ...
    try {
      doc.querySelectorAll('[data-baseweb="calendar"]').forEach(cal => {
        ...
        // 화살표 아이콘의 svg <title> — 호버 시 "Left"/"Right" 툴팁으로 노출됨
        cal.querySelectorAll('svg title').forEach(ti => {
          const t = ti.textContent.trim();
          if (t === 'Left') ti.textContent = '이전 달';
          else if (t === 'Right') ti.textContent = '다음 달';
        });
      });
