...
# ── 카페24 ─────────────────────────────────────────────
def _cafe24_orders(cli, start, end, embed=None):
    """카페24 admin/orders 원본 주문 목록.
    embed='items'면 품목(주문상태 포함)을 inline으로 받는다."""
...
# 주문 단위 필드(cancel_date/return_confirmed_date)로는 취소와 반품이 구분되지 않는다
# (취소완료·반품완료·입금전취소 모두 동일하게 채워짐). 품목의 order_status로만 판정 가능.
#   R* = 반품(R40 반품완료 등) · C40 = 취소완료(결제 후) · C47~49/N00 = 입금전(미결제)
def _order_claim(order):
    """주문의 품목들로 (결제됨, 반품, 취소) 판정. 입금전/입금전취소는 미결제로 제외."""
    paid = has_ret = has_can = False
    for it in order.get("items", []):
        stx = it.get("status_text") or ""
        if "입금전" in stx:                 # 입금전 / 입금전취소 → 미결제
            continue
        paid = True
        st = it.get("order_status") or ""
        if st.startswith("R"):
            has_ret = True                 # 반품
        elif st.startswith("C"):
            has_can = True                 # 취소완료(결제 후 취소)
    return paid, has_ret, has_can
...
def cafe24_returns(year, months):
    ...
    for store, mall in CAFE24_MALLS.items():
        ...
        for m in months:
            ...
            orders = _cafe24_orders(cli, s, e, embed="items")
            buy = ret = can = 0
            for o in orders:
                paid, has_ret, has_can = _order_claim(o)
                if not paid:               # 입금전취소 등 미결제 → 구매 아님
                    continue
                buy += 1
                if has_ret:                # 반품 우선(반품·취소 중복 시 반품으로)
                    ret += 1
                elif has_can:
                    can += 1
            rows.append({"년": year, "월": m, "채널": "카페24", "브랜드": brand,
                         "구매건수": buy, "반품건수": ret, "취소건수": can})
    return rows
...
def coupang_market_returns(year, months):
    ...
    for m in months:
        ...
        rows.append({"년": year, "월": m, "채널": "쿠팡(일반)", "브랜드": "전체",
                     "구매건수": buy, "반품건수": len(ret_ids), "취소건수": 0})
    return rows
...
def smartstore_returns(year, months):
    ...
    for m in months:
        ...
        for i in range(0, len(ids), 100):
            ...
            for o in details:
                ...
                if not od.startswith(f"{year}-{m:02d}"):
                    continue
                # 취소는 구매로 치지 않음(결제 후 취소는 카페24와 달리 스마트스토어는 취소 상태로 분리)
                if st == "CANCELED" or cs.startswith("CANCEL"):
                    canc += 1
                    continue
                total += 1
        ...
        rows.append({"년": year, "월": m, "채널": "스마트스토어", "브랜드": "아자차",
                     "구매건수": total, "반품건수": retn, "취소건수": canc})
    return rows
...
def _ensure_table(cur):
    ...
    # 취소(결제 후 취소)를 반품과 분리해서 저장 — 기존 테이블엔 없을 수 있어 보강
    cur.execute("ALTER TABLE monthly_returns ADD COLUMN IF NOT EXISTS 취소건수 INT DEFAULT 0")
...
def save_monthly_returns(df):
    ...
    rows = [(int(r["년"]), int(r["월"]), r["채널"], r["브랜드"],
             int(r["구매건수"]), int(r["반품건수"]), int(r.get("취소건수", 0) or 0))
            for _, r in df.iterrows()]
    psycopg2.extras.execute_values(cur,
        """INSERT INTO monthly_returns (년,월,채널,브랜드,구매건수,반품건수,취소건수) VALUES %s
           ON CONFLICT (년,월,채널,브랜드) DO UPDATE SET
           구매건수=EXCLUDED.구매건수, 반품건수=EXCLUDED.반품건수, 취소건수=EXCLUDED.취소건수""", rows)
...
def load_monthly_returns(year):
    conn = _get_conn(); cur = conn.cursor()
    _ensure_table(cur)
    cur.execute("""SELECT 년,월,채널,브랜드,구매건수,반품건수,COALESCE(취소건수,0)
                   FROM monthly_returns WHERE 년=%s""", (year,))
    data = cur.fetchall(); conn.close()
    return pd.DataFrame(data, columns=["년", "월", "채널", "브랜드", "구매건수", "반품건수", "취소건수"])
